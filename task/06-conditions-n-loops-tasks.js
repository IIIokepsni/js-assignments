...
function getDigitalRoot(num) {
    ...
    return sumOfDigits(sumOfDigits(num.toString()).toString());
}
        ...
        for (var j = 0; j < pathes.length; j++) {
            if(pathes[0][i] == pathes[j][i]) {
                if(j == pathes.length-1) {
                    intervalStr += pathes[0][i];
                    if(pathes[0][i] == '/'){
                        returnStr += intervalStr;
                        intervalStr = '';
                    }
                }
            }
...
function evaluateTicTacToePosition(position) {
    var cross = 'X', zero = '0', size = 3;
    function winRes(count)
    {
        return count == size ? true : false;
    }
    function searchWinner(symb) {
        var count = 0;
        if(!winRes(count)) {
            for (var i = 0; i < size; i++) {
                count = position[i][i] == symb ? count + 1 : count;
            }
        }
        if(!winRes(count)) {
            count = 0;
            for (var i = 0; i < size; i++) {
                count = position[i][(size-1)-i] == symb ? count + 1 : count;
            }
        }
        if(!winRes(count)) {
            count = 0;
            for (var i = 0; i < size; i++) {
                ...
                {
                    count = position[i][j] == symb ? count + 1 : count;
                    if(winRes(count))
                    {
                        return true;
                    }
        ...
        if(!winRes(count)) {
            ...
            for (var i = 0; i < size; i++) {
                ...
                for(var j = 0; j < size; j++)
                {
                    count = position[j][i] == symb ? count + 1 : count;
                    if(winRes(count))
                    {
                        return true;
                    }
    ...
    return searchWinner(zero) ? zero
            : searchWinner(cross) ? cross
                : undefined;
}
